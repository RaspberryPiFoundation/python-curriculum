...
Write out the code and see what it does, does it draw the right shape?. It should look like this
...
You can see we draw the second shape, turn, draw it again, turn, draw it again, turn and draw it one last time. Instead of writing all of the smaller
movements, let's draw it by calling `second`
...
Try running `snowflake(1, 50)`, `snowflake(2, 25)`, `snowflake(3, 20)`. The more depth, the longer it takes to draw, so remember to put `speed(0)` in to make the turtle rush!
...
def box(depth, length):
    if depth == 1: 
        ...
        # What should go here?
        # Copy the bumpy line code here, but use box(newdepth, length) instead of forward(100)
...
Let's draw one last fractal. It looks like a triforce, and like before we have a simple case and a special case.
...
If you like, instead of triangles we can draw it with circles!
